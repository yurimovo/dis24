...
import { deleteFcat } from '../../../../service-functions/fcats/deleteFcat';
...
const FcatRow: React.FC<IFcat> = observer (({ fcat, idx, reloadPage }) => {
    const { changeActiveFcat, changeEditingFcat } = store;
    ...
    const deleteFcatWithConfirmation = async (id: number) => {
        const userConfirmed = window.confirm("Вы уверены, что хотите удалить категорию?");
        if (userConfirmed) {
            try {
                const result = await deleteFcat(id);
                toast.success(`Вид охраны ${fcat.category_name} удален`, {
                    onClose: () => { 
    ...
    const handleDeleteClick = async (e: React.MouseEvent, id: number) => {
        e.stopPropagation();
        try {
            await deleteFcatWithConfirmation(id);
        } catch (error) {
            console.error('Ошибка при удалении категории:', error);
            toast.error(`Ошибка удаления категории ${fcat.category_name}`);
        }
    };

    return (
        <div className='container fcatRowContainer' /* onClick={changeId} */>
            <div className='row fcatRow'>
